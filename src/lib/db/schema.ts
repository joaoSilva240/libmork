...
export const spells = pgTable(
  "spells",
  {
    ...
    description: text("description"),
    useType: varchar("use_type", { length: 250 }).notNull().default("somatic"),
    duration: varchar("duration", { length: 100 }),
    extraEffect: text("extra_effect"),
    actionCostOverride: integer("action_cost_override"),
    imageUrl: varchar("image_url", { length: 500 }),
    range: varchar("range", { length: 250 }),
    target: text("target"),
    area: varchar("area", { length: 250 }),
    damage: jsonb("damage"),
    damageType: varchar("damage_type", { length: 100 }),
    structuredEffects: jsonb("structured_effects"),
    castingTime: varchar("casting_time", { length: 100 }),
...
export const items = pgTable(
  "items",
  {
    ...
    sourceKey: varchar("source_key", { length: 500 }).unique(),
    imageUrl: varchar("image_url", { length: 500 }),
    sourceData: jsonb("source_data"),
    translation: jsonb("translation"),
  },
  (table) => [index("idx_item_campaign").on(table.campaignId), index("idx_item_category").on(table.sourceKey)],
);
